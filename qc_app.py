...
def generate_report(info):
    ...
    # Data rows
    DATA_START = 4
    # Total NOK = all inspection NOK minus everything rework recovered.
    # Parts repaired in rework are no longer defective, so they must not count.
    # This yields exactly the rework NOK when every NOK part went through
    # rework, and stays correct when only part of them did. Clamped at 0
    # because some source files report more rework OK than inspection NOK.
    ...
    def col_ci(key):
        return next(i+1 for i,c in enumerate(cols) if c["key"]==key)

    def row_total_nok(rd):
        return max(0, sum(rd.get(k, 0) for k in insp_nok_keys)
                      - sum(rd.get(k, 0) for k in rw_ok_keys))
    ...
    for idx, row in enumerate(by_day.itertuples(index=False)):
        ...
        for ci, col in enumerate(cols, 1):
            ...
            if key == "date":
                ...
                nok_sum = "+".join(f"{get_column_letter(col_ci(k))}{r}" for k in insp_nok_keys)
                rw_sum  = "+".join(f"{get_column_letter(col_ci(k))}{r}" for k in rw_ok_keys)
                c.value = f"=MAX(0,({nok_sum})-({rw_sum}))" if rw_sum else f"={nok_sum}"
                c.number_format = "#,##0"; c.font = fnt(bold=True)
